...
- The startup membership check stays a startup-only operation. It is a full dialog scan, so re-running it on every file edit would be expensive and flood-wait-prone; a channel added at runtime is watched immediately but not membership-checked. The owner joins in Telegram; the Watchlist does not join it.
