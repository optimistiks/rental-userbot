...
- The owner's confirmation that an edit was picked up is a console log line, and it only appears on the next Post. Nothing is sent to Saved Messages about a configuration change.
- The startup membership check stays a startup-only operation. It is a full dialog scan, so re-running it on every file edit would be expensive and flood-wait-prone; a channel added at runtime is watched immediately but not membership-checked until the next restart.
