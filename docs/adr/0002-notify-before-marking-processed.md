...
- Identity is per Post, not per message: an album is one key built from its grouped ID. The key is checked again when the queue takes the Post, so a second copy that was queued before the first finished is dropped. A late album fragment therefore finds its album already processed and is dropped with it, losing its photos.
