...
- [ ] `login` command runs mtcute's interactive `start()` and creates the session file; it needs only `API_ID` and `API_HASH` and skips the other startup checks
...
- [ ] Dockerfile and compose file exactly as in § Deployment, including `WORKDIR /app`, the better-sqlite3 smoke check, the `node --import tsx` entrypoint, `USER node`, `init: true`, bind mount `./data:/app/data`; `.dockerignore` per spec
