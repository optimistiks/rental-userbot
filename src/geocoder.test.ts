...
describe('createGeocoder', () => {
  it('sends the LocationIQ query and maps results to the tool shape', async () => {
    ...
    await expect(geocoder.geocode('Gorgasali 33')).resolves.toEqual({ results: [
      {
        precision: 'building',
        lat: 41.6481086,
        lon: 41.6393883,
  ...
  it('throws a redacted provider error for an unauthorized response', async () => {
    ...
    await expect(geocoder.geocode('private')).rejects.toThrow('LocationIQ request failed (401): Invalid key')
  ...
  it('passes an abort signal through to fetch for the tool timeout', async () => {
    ...
    await expect(geocoder.geocode('slow', new AbortController().signal)).rejects.not.toThrow('secret-token')
