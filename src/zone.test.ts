...
const polygon = (name: string, coordinates = [
  [0, 0],
  [1, 0],
  [1, 1],
  [0, 1],
...
describe('readZoneFile', () => {
    ...
    const path = writeZone({
      ...
      features: [
        polygon('Old Batumi'),
        {
          ...
          geometry: { type: 'MultiPolygon', coordinates: [[[[2, 2], [3, 2], [3, 3], [2, 3], [2, 2]]]] },
        },
      ],
    })
  ...
  it('names a missing, malformed, or empty Zone file', () => {
    ...
    const mixedInvalidPath = writeZone({
      type: 'FeatureCollection',
      features: [polygon('Valid'), { type: 'Feature', properties: {}, geometry: { type: 'Polygon' } }],
    })
